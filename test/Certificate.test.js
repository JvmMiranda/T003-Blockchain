...
contract("Certificate", (accounts) => {
    ...
    it("deve falhar ao tentar registrar um certificado já existente", async () => {
        try {
            ...
            assert.include(error.message, "Certificate already registered", "O erro deve conter 'Certificate already registered'");
        }
    });
    ...
    it("deve falhar ao consultar um certificado inexistente", async () => {
        try {
            ...
            assert.include(error.message, "Certificate not found", "O erro deve conter 'Certificate not found'");
        }
    });
});
