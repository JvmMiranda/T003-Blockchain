// src/components/RegisterCertificate.tsx
...
const RegisterCertificate: React.FC = () => {
    ...
    const registerCertificate = async () => {
        ...
        try {
            ...
            // Convertendo o gas limit para string
            await certificateContract.methods.registerCertificate(id, studentName, course)
                .send({ from: accounts[0], gas: "3000000" }); // Gas limit como string

            setMessage("Certificado registrado com sucesso!");
        } catch (error) {
            console.error("Erro ao registrar o certificado:", error);
